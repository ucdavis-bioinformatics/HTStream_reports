...
mapping_dir = "02-BWA"
# log file and count file suffixes
logfile_suffix = ".ALL.flagstat"
countfile_suffix = ".ALL.counts"
...
process_json <- function(sdir) { 
    ...
    logfinal = read_file(paste0(mapping_dir,"/",sdir,"/",sdir,logfile_suffix))
    #lfall = str_match_all(logfinal,"Uniquely mapped reads number \\|\\t(\\d+)\\n.+Uniquely mapped reads \\% \\|\\t(.+\\%)")
    lfall = str_match_all(logfinal,"(\\d+) \\+ 0 mapped \\((.+\\%) :")
    
    lfm = c("Uniquely Mapped Reads"=lfall[[1]][,2], "Uniquely Mapped Perc"=lfall[[1]][,3])
...
process_counts <- function(sid, colnum) {
  counts = read.table(paste0(mapping_dir,"/",sid,"/",sid,countfile_suffix),row.names = 1,sep='\t',stringsAsFactors = F,header = F,skip=3)
  counts = counts[,6]
  return(counts)
}
...
counts = read.table(paste0(mapping_dir,"/",sample_dirs[1],"/",sample_dirs[1],countfile_suffix),row.names = 1,sep='\t',stringsAsFactors = F,header=F,skip=3)
